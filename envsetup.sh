...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^zz_") ; then
       CUSTOM_BUILD=$(echo -n $1 | sed -e 's/^zz_//g')
    else
       CUSTOM_BUILD=
    fi
    export CUSTOM_BUILD
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/zz/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the CrystalZZ model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch zz_$target-$variant
        fi
    fi
    return $?
}
